...
`progress.json` is treated as untrusted JSON of unknown shape. Every field is read with `?.`/`??`, an unknown `status` or `workflow[].step` renders verbatim in neutral styling, and the **Raw** tab always shows the file exactly as read, with a copy button. Real folders on disk already carry values outside the documented set; a schema check here would hide jobs rather than help.
...
Only `PUT /api/config`, the two POST run routes and `/api/usage/refresh` change anything. Work Hub never writes to a monitored folder - the only files it writes are `~/.work-hub/config.json` and whatever `claude` itself writes.
...
- Editing `progress.json`, `PLAN.md`, or anything else under a monitored folder.
